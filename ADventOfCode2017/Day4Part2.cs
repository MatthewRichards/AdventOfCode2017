...
namespace AdventOfCode2017
{
    public class Day4Part2 : ISolution
    {
        public int Solve()
        {
            ...
            int validPassphrases = wordsInPassphrases.Count(wordsInPassphrase => !ContainsAnagrams(wordsInPassphrase));

            return validPassphrases;
        }

        private bool ContainsAnagrams(string[] words)
        {
            var wordsWithSortedLetters = words.Select(word => new string(word.OrderBy(chr => chr).ToArray())).ToArray();
            return wordsWithSortedLetters.Distinct().Count() != words.Length;
        }

        private static string Input = @"oaoe rxeq vssdqtu xrk cjv yaoqp loo
