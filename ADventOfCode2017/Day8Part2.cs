...
{
    public class Day8Part2 : ISolution
    {
        public int Solve()
        {
            var instructions = Input.SplitByNewLines().Select(line => InstructionRegex.Match(line));
            var registers = new Dictionary<string, int>();
            var largestEverValue = 0;

            foreach (var instruction in instructions)
            {
                var registerToModify = instruction.Groups["targetreg"].Value;
                var registerToTest = instruction.Groups["testreg"].Value;
                var valueToTestAgainst = registers.ContainsKey(registerToTest) ? registers[registerToTest] : 0;
                var valueToTest = int.Parse(instruction.Groups["testval"].Value);
                var testSucceeded = false;

                switch (instruction.Groups["cond"].Value)
                {
                    case ">":
                        testSucceeded = valueToTestAgainst > valueToTest;
                        break;
                    ...
                    default:
                        throw new InvalidOperationException($"Unable to parse condition in {instruction.Value}");
                }

                    var valueToChangeBy = testSucceeded ? int.Parse(instruction.Groups["val"].Value) : 0;

                if (instruction.Groups["op"].Value == "dec") valueToChangeBy = -valueToChangeBy;

                registers[registerToModify] = (registers.ContainsKey(registerToModify) ? registers[registerToModify] : 0) + valueToChangeBy;

                if (registers[registerToModify] > largestEverValue)
                {
                    largestEverValue = registers[registerToModify];
                }
            }

            return largestEverValue;

        }

        // This site seems pretty good: http://regexstorm.net/tester
        private static Regex InstructionRegex = new Regex(@"(?<targetreg>\w+) (?<op>inc|dec) (?<val>\-?\d+) if (?<testreg>\w+) (?<cond>\>|\<||\<\=|\!\=|\=\=|\>\=) (?<testval>-?\d+)");
 
        private static string TestInput = @"b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10";
        private static string Input = @"ioe dec 890 if qk > -10
gif inc -533 if qt <= 7
itw dec 894 if t != 0
