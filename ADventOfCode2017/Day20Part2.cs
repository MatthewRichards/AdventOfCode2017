...
namespace AdventOfCode2017
{
  ...
  public class Day20Part2 : ISolution
  {
    public int Solve()

    {
      var particleDefinitions = Input.SplitByNewLines();
      var particles = new List<Particle>();

      foreach (var particleDefinition in particleDefinitions)
      {
        var match = ParticleRegex.Match(particleDefinition);
        var position = Tuple.Create(
          int.Parse(match.Groups["p1"].Value),
          int.Parse(match.Groups["p2"].Value),
          int.Parse(match.Groups["p3"].Value));
        var velocity = Tuple.Create(
          int.Parse(match.Groups["v1"].Value),
          int.Parse(match.Groups["v2"].Value),
          int.Parse(match.Groups["v3"].Value));
        var acceleration = Tuple.Create(
          int.Parse(match.Groups["a1"].Value),
          int.Parse(match.Groups["a2"].Value),
          int.Parse(match.Groups["a3"].Value));

        var particle = new Particle
        {
          AccelerationX = acceleration.Item1,
          AccelerationY = acceleration.Item2,
          AccelerationZ = acceleration.Item3,
          PositionX = position.Item1,
          PositionY = position.Item2,
          PositionZ = position.Item3,
          VelocityX = velocity.Item1,
          VelocitY = velocity.Item2,
          VelocityZ = velocity.Item3
        };

        particles.Add(particle);
      }

      const int foolishLargeNumber = 20000;
      ...
      for (int i = 0; i < foolishLargeNumber; i++)
      {
        ...
        foreach (var particle in particles)
        {
          ...
          particle.VelocitY += particle.AccelerationY;
          particle.VelocityZ += particle.AccelerationZ;
          particle.PositionX += particle.VelocityX;
          particle.PositionY += particle.VelocitY;
          particle.PositionZ += particle.VelocityZ;
        }

        var collidingParticles = new List<Particle>();
        foreach (var particle in particles)
        {
          foreach (var otherParticle in particles.Where(p => p != particle && p.PositionX == particle.PositionX))
          {
            if (particle.PositionX == otherParticle.PositionX &&
                particle.PositionY == otherParticle.PositionY &&
                particle.PositionZ == otherParticle.PositionZ)
            {
        ...
        particles.RemoveAll(p => collidingParticles.Contains(p));

      }


      var distancesFromOrigin = particles.Select(part => Math.Abs(part.PositionX) + Math.Abs(part.PositionY) + Math.Abs(part.PositionZ)).ToArray();
      var minDistance = distancesFromOrigin.Min();

      Console.WriteLine($"Min distance: {minDistance}, particle {Array.IndexOf(distancesFromOrigin, minDistance)}"); 
      return particles.Count;
    }
 

private static Regex ParticleRegex = new Regex(@"p=\<(?<p1>-?\d+),(?<p2>-?\d+),(?<p3>-?\d+)\>, v=\<(?<v1>-?\d+),(?<v2>-?\d+),(?<v3>-?\d+)\>, a=\<(?<a1>-?\d+),(?<a2>-?\d+),(?<a3>-?\d+)\>", RegexOptions.Compiled);
    private static string TestInput = @"p=<3,0,0>, v=<2,0,0>, a=<-1,0,0>
...
private static string Input = @"p=<-1027,-979,-188>, v=<7,60,66>, a=<9,1,-7>
